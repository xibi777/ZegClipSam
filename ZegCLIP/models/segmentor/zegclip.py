...
class ZegCLIP(EncoderDecoder):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, mask):
        ...
        # average the same semantic patches
        mask_patch = self.mask_tensor_patch(mask)
        visual_ave_embedding = self.average_features_by_mask(mask_patch, visual_feat[0][0])
        visual_ave_embedding = (visual_ave_embedding,)
        visual_ave_feat = (visual_ave_embedding, visual_feat[1])
        ...
        # use average visual feature
        feat.append(visual_ave_feat)
    ...
    def simple_test(self, img, img_meta, mask, rescale=True):
        """Simple test with single image."""
        seg_logit = self.inference(img, img_meta, mask[0], rescale)
    ...
    def inference(self, img, img_meta, mask, rescale):
        ...
        if self.test_cfg.mode == 'slide':
            seg_logit = self.slide_inference(img, img_meta, mask, rescale)
        else:
            seg_logit = self.whole_inference(img, img_meta, mask, rescale)
    ...
    def encode_decode(self, img, img_metas, mask):
        visual_feat = self.extract_feat(img)
    ...
    # TODO refactor
    def slide_inference(self, img, img_meta, mask, rescale):
