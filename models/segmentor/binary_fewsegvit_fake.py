...
class BinaryFakeFewSegViT(FewEncoderDecoder):
    ...
    def __init__(self,
                ...
                #  init_cfg=None,
                 **args):
        super(BinaryFakeFewSegViT, self).__init__(**args)
    ...
    def forward(self, img, img_metas, return_loss=True, **kwargs):
        if return_loss:
            ...
            if len(self.base_class) != len(self.both_class): #generalized few-shot setting
                self.novel_queries = self.extract_novel_proto(self.supp_dir, self.supp_path)
                self.pair_test += 1
