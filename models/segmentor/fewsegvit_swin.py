...
class SwinFewSegViT(FewEncoderDecoder):
    ...
    def __init__(self,
                ...
                #  init_cfg=None,
                 **args):
        super(SwinFewSegViT, self).__init__(**args)
        ...
        for filename in f:
            ...
            try: image = image.unsqueeze(0).to(self.backbone.class_token.device)
                ...
                try: image = image.unsqueeze(0).to(self.backbone.cls_token.device)
                    ...
                    try: image = image.unsqueeze(0).to(self.backbone.fc.weight.device)
                    except: image = image.unsqueeze(0).to(self.backbone.patch_embed.projection.weight.device)
    ...
    def extract_novel_multi_proto(self, dir, path, way, shot):
        ## load Image and Annotations, no augmentation but how to handle the crop??
        ## seed from GFS-Seg: 5
        all_novel_queries = np.zeros([way, self.decode_head.use_stages, self.decode_head.in_channels]) # [way, 3, dim]
        ...
        for filename in f:
            ...
            for i_stage in range(self.decode_head.use_stages):
                ...
                # obtain the mask
                patch_token_cls_i = F.interpolate(patch_token_cls_i, size=label.shape[-2:], mode='bilinear', align_corners=False) ## (512, 512)
