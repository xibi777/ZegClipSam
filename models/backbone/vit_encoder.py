...
class MaskMultiHeadedSelfAttention(nn.Module):
    ...
    def prepocess(self, masks, scores_ori, v_ori, x_ori): # repeat scores
        ...
        for i in range(len(masks)):
            ...
            new_scores_i = scores_ori[i].unsqueeze(0).repeat(masks_i.shape[0], 1, 1, 1)
            new_v_i = v_ori[i].unsqueeze(0).repeat(masks_i.shape[0], 1, 1, 1)
            new_x_i = x_ori[i].unsqueeze(0).repeat(masks_i.shape[0], 1, 1)
            # new_masks.append(new_masks_i)
            if i == 0:
                new_masks = new_masks_i
                new_scores = new_scores_i
                new_v = new_v_i
                new_x = new_x_i
            else:
                new_masks = torch.concat([new_masks, new_masks_i], dim=0)
                new_scores = torch.concat([new_scores, new_scores_i], dim=0)
                new_v = torch.concat([new_v, new_v_i], dim=0)
                new_x = torch.concat([new_x, new_x_i], dim=0)
        ...
        new_scores = new_scores.reshape(len(img_idx), head, dim, dim).to(masks_i.dtype).to(masks_i.device)
        new_v = new_v.reshape(len(img_idx), head, dim, v_ori.shape[-1]).to(v_ori.dtype).to(v_ori.device)
        new_x = new_x.reshape(len(img_idx), dim, x_ori.shape[-1]).to(x_ori.dtype).to(x_ori.device)
