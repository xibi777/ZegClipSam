...
class CrossATMSingleHeadSeg(BaseDecodeHead):
    def __init__(
            self,
            img_size,
            in_channels,
        ...
        super(CrossATMSingleHeadSeg, self).__init__(
    ...
    def forward(self, inputs, qs_epoch=None, novel_queries=None):
        ...
        if not self.training:
            ...
            both_proto = torch.from_numpy(novel_queries).to(patch_token[0].device).to(patch_token[0].dtype) # do not use the original base prototypes
        
        assert both_proto.shape[0] == len(self.novel_idx)
