...
class MultiATMSingleHeadSeg(BaseDecodeHead):
    ...
    def init_proto(self):
        ...
        init_protos = torch.from_numpy(np.load(path)).to(self.base_qs.dtype).to(self.base_qs.device)[:, -self.use_stages:][self.seen_idx]
        self.base_qs.data = init_protos #(16, 5, 768)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, qs_epoch, train_cfg, mask):
        seg_logits = self.forward(inputs, qs_epoch=qs_epoch, mask=mask)
    ...
    def forward_test(self, inputs, img_metas, test_cfg, novel_queries=None, mask=None):
        if novel_queries is not None:
            return self.forward(inputs, novel_queries=novel_queries)
        else:
            return self.forward(inputs)
    ...
    def mask_tensor_patch(self, mask, patch_size=16):
        batch_size, _, h, w = mask.shape
        assert h % patch_size == 0 and w % patch_size == 0
        out_h, out_w = h // patch_size, w // patch_size
        mask_patch = torch.zeros((batch_size, out_h, out_w), dtype=mask.dtype, device=mask.device)
        for b in range(batch_size):
            for i in range(out_h):
                for j in range(out_w):
                    patch = mask[b, 0, i * patch_size:(i + 1) * patch_size, 
                                 j * patch_size:(j + 1) * patch_size]
                    if torch.all(patch == patch[0, 0]):
                        mask_patch[b, i, j] = patch[0, 0]
                    else:
                        mask_patch[b, i, j] = 255
        return mask_patch
    
    def average_features_by_mask(self, masks, visual_feats):
        B, _, _, _ = visual_feats.shape
        visual_ave_feat = visual_feats.clone()
        for b in range(B):
            mask = masks[b]
            visual_feat = visual_feats[b]
            unique_vals = torch.unique(mask)
            unique_vals = unique_vals[unique_vals != 255]
            if unique_vals.numel() != 0:
                for val in unique_vals:
                    mask_region = (mask == val)
                    if mask_region.sum() > 0:
                        avg_feature = visual_feat[:, mask_region].mean(dim=1)
                        visual_ave_feat[b][:, mask_region] = avg_feature.unsqueeze(1)
            else:
                visual_ave_feat[b] = visual_feats[b]
        
        return visual_ave_feat

    def forward(self, inputs, qs_epoch=None, novel_queries=None, mask=None):
        # # average inputs by sam
        # mask_patch = self.mask_tensor_patch(mask)
        # visual_ave_embedding = []
        # for i_patch_token in range(self.use_stages-1):
        #     visual_ave_embedding_i = self.average_features_by_mask(mask_patch, inputs[0][0][i_patch_token][1])
        #     visual_ave_embedding.append(visual_ave_embedding_i)
        # visual_ave_embedding_last = self.average_features_by_mask(mask_patch, inputs[0][0][-1])
        # visual_ave_embedding.append(visual_ave_embedding_last)
        ...
        if self.use_stages == 1:
            ...
            # # use average embedding from sam
            # patch_token = torch.stack([visual_ave_embedding[i_stage] for i_stage in range(self.use_stages-1)])
            # patch_token = torch.concat([patch_token, visual_ave_embedding[-1].unsqueeze(0)])
            
            # commented-out by yuqi
            ## combine the patch_token from different layers
            patch_token = torch.stack([inputs[0][0][i_stage][1] for i_stage in range(self.use_stages-1)])
