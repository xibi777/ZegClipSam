...
class TPN_DecoderLayer(TransformerDecoderLayer):
    def __init__(self, tune, **kwargs):
        ...
        self.multihead_attn = TuneAttention(
            kwargs['d_model'], num_heads=kwargs['nhead'], qkv_bias=True, attn_drop=0.1, tune=tune)
...
#         attn = (q @ k.transpose(-2, -1)) * self.scale #(1, 10, 512) (1, )
...
class TuneAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., tune=False):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        # NOTE scale factor was wrong in my original version, can set manually to be compat with prev weights
        ...
        if tune:
            self.tune_k = nn.Parameter(nn.init.ones_(torch.FloatTensor(dim)), requires_grad=True)
            self.tune_v = nn.Parameter(nn.init.ones_(torch.FloatTensor(dim)), requires_grad=True)
            self.tune_f = nn.Parameter(nn.init.ones_(torch.FloatTensor(dim)), requires_grad=True)

    def forward(self, xq, xk, xv):
        if self.tune:
            out, attn = self.tune_forward(xq, xk, xv)
        else:
            out, attn = self.ori_forward(xq, xk, xv)
        return out, attn

    def ori_forward(self, xq, xk, xv):
        B, Nq, C = xq.size() # 1, 21, 512
        Nk = xk.size()[1]
        Nv = xv.size()[1]
    ...
    def tune_forward(self, xq, xk, xv):
        B, Nq, C = xq.size() # 1, 21, 512
        Nk = xk.size()[1]
        Nv = xv.size()[1]
        ## tune k
        # print('k:', self.tune_k.mean())
        # # print('k:', self.tune_k.requires_grad)
        # print('v:', self.tune_v.mean())
        # # print('v:', self.tune_v.requires_grad)
        # print('f:', self.tune_f.mean())
        # print('f:', self.tune_f.requires_grad)
        

        q = self.q(xq).reshape(B, Nq, self.num_heads,
                                      C // self.num_heads).permute(0, 2, 1, 3)
        k = (self.tune_k * self.k(xk)).reshape(B, Nk, self.num_heads,
                                      C // self.num_heads).permute(0, 2, 1, 3)
        v = (self.tune_v * self.v(xv)).reshape(B, Nv, self.num_heads,
                                      C // self.num_heads).permute(0, 2, 1, 3)
...
class FTATMSingleHeadSeg(BaseDecodeHead):
    def __init__(
            self,
            img_size,
            in_channels,
        ...
        super(FTATMSingleHeadSeg, self).__init__(
        ...
        for i in range(self.use_stages):
            ...
            # decoder layer
            if tune == 'kvf':
                decoder_layer = TPN_DecoderLayer(d_model=self.dim, nhead=nhead, dim_feedforward=self.dim * 4, tune=True)
            else:
                decoder_layer = TPN_DecoderLayer(d_model=self.dim, nhead=nhead, dim_feedforward=self.dim * 4, tune=False)
        ...
        if tune == 'proto':
            self.proto_proj = nn.Parameter(nn.init.ones_(torch.FloatTensor(embed_dims)), requires_grad=True)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, qs_both, train_cfg):
        seg_logits = self.forward(inputs, qs_both)
    ...
    def forward_test(self, inputs, img_metas, qs_both, test_cfg):
        return self.forward(inputs, qs_both)

    def forward(self, inputs, qs_both):
        patch_token = inputs[0][0]
        cls_token = inputs[0][1]

        if self.tune == 'proto':
            qs_both = self.proto_proj * qs_both
        ...
        q = self.q_proj(self.get_qs(qs_both, cls_token)).transpose(0, 1)
