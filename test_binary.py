...
def parse_args():
    ...
    parser.add_argument('qry_path', help='the npy path of query set')
    parser.add_argument(
        '--work-dir',
...
def main():
    ...
    num_novel_classes = int(len(model.CLASSES) / 4)
    print('Number of Novel Classes:', num_novel_classes)
    split = int(args.qry_path.split('/').pop(-1).split('_').pop(2).split('.').pop(0))
    print('Split:', split)
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            mIoU = dataset.evaluate(results, args.qry_path, num_novel_classes, split,**eval_kwargs)

            path = args.checkpoint.split('/')
            for nn in range(len(path)-1):
                if nn==0:
                    savepath = str(path[nn]) + '/'
                else:
                    savepath = savepath+str(path[nn]) + '/'
            iter_num = path.pop(-1).split('_').pop(-1).split('.').pop(0)
            shot = args.config.split('_').pop(-2)
            savepath = savepath + 'binary_fsseg_' + str(shot) + '_' + str(iter_num) + '.txt'

            print('Save Path:', savepath)
            
            f = open(savepath, 'a+')
            f.write(args.qry_path + '\n')
